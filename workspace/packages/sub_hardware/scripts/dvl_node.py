...
# Below this altitude, DVL bottom-track velocity becomes unreliable and is
# withheld from the EKF (multipath/near-field acoustic returns).
MIN_ALTITUDE_FOR_VELOCITY_M = 0.25
...
class DVLNode(Node):
    ...
    def _handle_velocity_report(self, fields: list[str]):
        ...
        velocity_rejected = altitude <= MIN_ALTITUDE_FOR_VELOCITY_M
        self.velocity_rejected_pub.publish(Bool(data=velocity_rejected))

        if not velocity_rejected:
            self.velocities_pub.publish(twist_msg)
        else:
            self.get_logger().warn(
                f'Altitude {altitude:.2f}m at or below {MIN_ALTITUDE_FOR_VELOCITY_M}m minimum; '
                'withholding velocity from EKF.',
                throttle_duration_sec=1.0
            )
